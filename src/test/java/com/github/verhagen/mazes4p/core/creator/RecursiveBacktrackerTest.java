...
public class RecursiveBacktrackerTest {
	...
	public void grid(int rows, int columns) {
		...
		summary.setMazeCreator(new RecursiveBacktracker());
		...
		Graphics2DArt graphicsVisitor = new Graphics2DArt2WithDistance(imagePath, "recursive-backtracker", rows, columns, summary
				, 10, midGridDistances, colorIndex);
		grid.accept(graphicsVisitor);

		Graphics2DHtml graphics2dHtml = new Graphics2DHtmlWithDistance(htmlPath, "recursive-backtracker", rows, columns, summary
				, 10, midGridDistances, colorIndex);
